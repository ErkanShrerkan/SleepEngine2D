			...
			if (ImGui::BeginMenu(category.c_str()))
			{
				...
				for (auto& variable : pair.second)
				{
					...
					case JsonExposedType::Bool:
						if (variable.imguiTag == JsonExposedImguiTag::Button)
						{/*
							float width = ImGui::CalcItemWidth();*/
							if (ImGui::Button(("Button" + variable.labelId).c_str()))
							{
								for (auto& address : variable.addresses)
									TREAT_AS(bool, address) = true;
							}
						}
						else
						{
...
bool JsonManager::MeshHasAppliedTexture(const std::string& aModelPath, int aMeshIndex, std::string& aNameBuffer)
{
	auto& doc = myJsonDocument->GetDocument();

	std::string saneName(aModelPath);
	std::replace(saneName.begin(), saneName.end(), '/', '\\');

	std::string meshIndex = "Mesh_" + std::to_string(aMeshIndex);
	if (rapidjson::Pointer(("/TextureSets/" + saneName + "/" + meshIndex).c_str()).Get(doc))
	{
		aNameBuffer = rapidjson::Pointer(("/TextureSets/" + saneName + "/" + meshIndex).c_str()).Get(doc)->GetString();
		return true;
	}
...
void JsonManager::ExposeVariable(void* anAddress, JsonExposedType aType, const std::string& aLabel, const std::string& anOverrideLabel, const std::string& aCategory, const JsonExposedImguiTag& anImGuiTag, const std::string& aLabelSuffix)
{
	...
	if (aLabelSuffix != "")
	{
		variable.labelId += "/$" + aLabelSuffix;
		variable.labelText += " (" + aLabelSuffix + ")";
	}
