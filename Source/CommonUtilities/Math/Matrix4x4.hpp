...
namespace CommonUtilities
{
	...
	template<class T>
	inline Matrix4x4<T> Matrix4x4<T>::Inverse()
	{
		T inv[16]{}, det{};
		int i;

		Matrix4x4<T> t = Matrix4x4<T>::Transpose(*this);
	...
	template<class T>
	inline Matrix4x4<T> Matrix4x4<T>::Normalized()
	{
	...
	template<class T>
	inline Vector3<T> Matrix4x4<T>::GetScale()
	{
		T rightLength = GetRight().Length();
		T upLength = GetUp().Length();
		T forwardLength = GetForward().Length();
		return { rightLength, upLength, forwardLength };
	}
