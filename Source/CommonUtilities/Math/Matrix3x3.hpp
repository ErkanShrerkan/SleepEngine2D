...
namespace CommonUtilities
{
	...
	{
		(*this)(aOneBasedIndex, 1) = aVector3.x;
		(*this)(aOneBasedIndex, 2) = aVector3.y;
		(*this)(aOneBasedIndex, 3) = aVector3.z;
	}
	template<class T>
	inline Vector3<T>& Matrix3x3<T>::GetRow(int aOneBasedindex) const
	{
		return *reinterpret_cast<Vector3<T>*>(const_cast<T*>(&myMatrix[aOneBasedindex - 1][0]));
	}
	template<class T>
	inline Vector2<T>& Matrix3x3<T>::GetPosition()
	{
		return GetRow(3).xy;
	}
	template<class T>
