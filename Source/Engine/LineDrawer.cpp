...
namespace SE
{
	...
	void CLineDrawer::DrawLine(float3 aStart, float3 anEnd, float4 aCol)
	{
		if (myIsEnabled)
		{
			AddVert({ {aStart, 1}, aCol });
			AddVert({ {anEnd, 1}, aCol });
		}
	}
...
namespace Debug
{
	void DrawLine2D(float2 aStart, float2 anEnd, float4 aCol)
	{
		SE::CLineDrawer::DrawLine({ aStart.x, aStart.y, .5f }, { anEnd.x, anEnd.y, .5f }, aCol);
	}
}