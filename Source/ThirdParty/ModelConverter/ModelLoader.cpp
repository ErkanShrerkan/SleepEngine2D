...
struct float3
{
	...
	float3(float _x, float _y, float _z)
		...
		, z(_z)
	{}
	float3(const float3& v1, const float3& v2, const float3& v3)
	{
		x = v1.x;
		y = v2.y;
		z = v3.z;
		...
		if (hasBones)
		{
			...
			for (int deformIdx = 0; deformIdx < fbxMesh->GetDeformerCount(); deformIdx++)
			{
				...
				for (int clusterIdx = 0; clusterIdx < fbxSkin->GetClusterCount(); clusterIdx++)
				{
					...
					size_t jointIndex = mdlSkeleton.myJointNameToIndex[CommonUtilities::Hash(std::string(fbxCluster->GetLink()->GetName()))];
...
std::string ModelLoader::GetRootJoint(std::unordered_map<unsigned, std::vector<unsigned>>& aJointHierarchy)
{
...
bool ModelLoader::LoadAnimation(void* aScene, std::string aPath)
{
	...
	std::unordered_map<unsigned, std::vector<unsigned>> jointHierarchy;
	unsigned rootName = CommonUtilities::Hash(std::string(scene->mRootNode->mName.C_Str()));
	...
	for (int i = 0; i < scene->mAnimations[0]->mNumChannels; i++)
	{
		...
		anim.AddJoint(CommonUtilities::Hash(nodeName));
		ModelLoader::Animation::Track& track = anim.GetTrack(CommonUtilities::Hash(nodeName));
	...
	wf.write((char*)&size, sizeof(UINT));
	wf.write((char*)&rootName, sizeof(UINT));
	for (auto& joint : jointHierarchy)
	{
		wf.write((char*)&joint.first, sizeof(UINT));
		unsigned numChildren = joint.second.size();
		wf.write((char*)&numChildren, sizeof(UINT));
		for (auto& child : joint.second)
		{
			//wf.write(child.c_str(), 32);
			wf.write((char*)&child, sizeof(UINT));
		}
	}
	...
	{
		wf.write((char*)&node.first, sizeof(UINT));
	...
	{
		wf.write((char*)&track.first, sizeof(UINT));

		size = track.second.myTranslationKeys.size();
...
void ModelLoader::ReadNodeHierarchy(void* aScene, void* aNode, std::unordered_map<unsigned, std::vector<unsigned>>& aNodeHierarchy, std::string aParentName, bool aParentWasNull)
{
	...
	if (!aParentWasNull)
	{
		...
		//if (/*nodeName.find_first_of('$') != std::string::npos*/false)
		//{
		...
		//	}
		//}
		//if (/*newParentName.find_first_of('$') != std::string::npos*/false)
		//{
		...
		myNodeTransforms[CommonUtilities::Hash(nodeName)] = transform;
		aNodeHierarchy[CommonUtilities::Hash(aParentName)].push_back(CommonUtilities::Hash(nodeName));
	}
...
bool ModelLoader::GatherSkeletonData(FbxNode* aNode, Skeleton& aSkeleton, unsigned anIndex, unsigned aParentIndex)
{
	if (aNode->GetNodeAttribute() && aNode->GetNodeAttribute()->GetAttributeType() && aNode->GetNodeAttribute()->GetAttributeType() == FbxNodeAttribute::eSkeleton)
	{
		...
		std::string name(aNode->GetName());
		joint.myName = CommonUtilities::Hash(name);
		//_memccpy(&joint.myName[0], aNode->GetName(), '\0', 32);
		aSkeleton.myJoints.push_back(joint);
		aSkeleton.myJointNameToIndex.insert({ joint.myName, aSkeleton.myJoints.size() - 1 });
	}
...
bool ModelLoader::LoadFBX(std::string aPath, int aMode)
{
	...
	float3 minPoint;
	minPoint.x =  9999999999999999999999.f;
	minPoint.y =  9999999999999999999999.f;
	minPoint.z =  9999999999999999999999.f;
	float3 maxPoint;
	maxPoint.x = -9999999999999999999999.f;
	maxPoint.y = -9999999999999999999999.f;
	maxPoint.z = -9999999999999999999999.f;
	...
	for (unsigned meshIndex = 0; meshIndex < aiScene->mNumMeshes; meshIndex++)
	{
		...
		if (aiMesh->HasBones())
		{
			...
			for (unsigned int i = 0; i < aiMesh->mNumBones; i++)
			{
				...
				if (skeleton.myJointNameToIndex.find(CommonUtilities::Hash(BoneName)) == skeleton.myJointNameToIndex.end())
				{
					...
					joint.myName = CommonUtilities::Hash(BoneName);
					memcpy(&(joint.myIndex), &boneIndex, sizeof(int));

					skeleton.myJointNameToIndex[CommonUtilities::Hash(BoneName)] = boneIndex;
					skeleton.myJoints.push_back(joint);
				}
				else
				{
					boneIndex = skeleton.myJointNameToIndex[CommonUtilities::Hash(BoneName)];
				}
	...
	std::ofstream wf(filePath.c_str(), std::ios::out | std::ios::binary);
	wf.write((char*)&size, sizeof(UINT));
	wf.write((char*)&minPoint.x, sizeof(float) * 3);
	wf.write((char*)&maxPoint.x, sizeof(float) * 3);
	wf.write((char*)&hasBones, sizeof(bool));
