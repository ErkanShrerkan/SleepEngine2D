...
class ModelLoader
{
	...
	struct Skeleton
	{
		...
		std::unordered_map<unsigned, unsigned> myJointNameToIndex;
	};
	...
	public:
		...
		std::unordered_map<unsigned, Track> myAnimation;
		float myDurationInFrames;
		float myDurationInSeconds;
		float myFPS;

	public:
		void AddJoint(unsigned aJointName) { myAnimation[aJointName]; }
		Track& GetTrack(unsigned aJointName) { return myAnimation[aJointName]; }
		bool JointExists(unsigned aJointName)
		{
		...
		std::unordered_map<unsigned, Track>& GetTracks()
		{
			return myAnimation;
		}
	};
	...
	void ReadNodeHierarchy(
		...
		std::unordered_map<unsigned, std::vector<unsigned>>& aNodeHierarchy,
	...
	std::string GetRootJoint(std::unordered_map<unsigned, std::vector<unsigned>>& aJointHierarchy);
...
public:
	...
	std::unordered_map<unsigned, Matrix> myNodeTransforms;
};
